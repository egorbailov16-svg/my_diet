...
Отвечай ТОЛЬКО валидным JSON по этой схеме:
{
  ...
  "predictions": [
    "1-2 пункта про изменение веса (направление + ориентировочный диапазон за 14 дней, если возможно)",
    "1-2 пункта про изменение формы/восстановление/самочувствие (особенно с учетом dayType и активности)"
  ],
...
КРИТИЧНО:
- При выводах о весе обязательно учитывай дефицит/профицит энергии. Если net-калории и/или съеденные калории ниже целевого диапазона, не пиши про набор веса как основной сценарий.
- В разделах "good" и "issues" обязательно дай явный разбор МАКРО (Б/Ж/У) и МИКРО/витаминов: минимум по 1 пункту про макро и 1 пункту про микро в каждом разделе, если данные есть.
- "predictions" должны быть согласованы с энергетическим балансом и уровнем активности.
- В predictions обязательно присутствуют: (а) прогноз веса, который следует из net-калорий относительно целевого диапазона, (б) прогноз по форме/восстановлению, который следует из dayType (normal/strength) и структуры рациона (доля белка/углеводов/жиров).`;
